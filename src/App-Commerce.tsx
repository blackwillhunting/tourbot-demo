...
function TourBarHotelBookingExtras({
  ...
  const matrixResults = Array.isArray(raw.matrixResults)
    ? raw.matrixResults.map(asRecord).filter((item) => item.roomId)
    : [];
  const showMatrixResults = !selected.roomId && matrixResults.length > 0;
  ...
  return (
    <div className="space-y-3">
      ...
      {showMatrixResults && (
        <div className="space-y-2 rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-2.5 text-sm text-slate-700">
          <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
            Matrix results
          </div>
          {matrixResults.slice(0, 4).map((item, index) => {
            const itemPricing = asRecord(item.pricing);
            const role = String(item.resultRole || "");
            const label =
              role === "lowest_price"
                ? "Lowest-priced room"
                : role === "highest_price"
            ...
            return (
              <div
                key={`${item.comboId || item.roomId || index}`}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 truncate font-semibold text-slate-950">
                      {item.roomShortTitle || item.roomTitle || item.roomId}
                    </div>
                    {item.view && (
                  ...
                  {itemPricing.effectiveNightlyUsd != null && (
                    <div className="shrink-0 rounded-full bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-900 ring-1 ring-slate-200">
                      ${Number(itemPricing.effectiveNightlyUsd).toLocaleString()}/night
                    </div>
                  )}
                </div>
              </div>
