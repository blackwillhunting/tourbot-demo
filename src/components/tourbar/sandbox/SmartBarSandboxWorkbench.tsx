...
import { ArrowLeft, MessageSquareText, ReceiptText, Send, XCircle } from "lucide-react";
...
export default function SmartBarSandboxWorkbench({ onBack }: SmartBarSandboxWorkbenchProps) {
  ...
  return (
    <div className="mx-auto mt-5 max-w-xl rounded-[28px] bg-white/92 p-3 shadow-[0_18px_44px_rgba(15,23,42,0.08)] ring-1 ring-white/80 sm:mt-7 sm:p-4">
      ...
      <form onSubmit={handleSubmit} className="mt-3 rounded-[24px] bg-slate-50 p-3 ring-1 ring-slate-200/80">
        <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-700">
          <MessageSquareText className="h-4 w-4 text-[#012169]" />
          SmartBar input
        </div>

        <div className="flex items-end gap-2 rounded-[22px] bg-white p-2 ring-1 ring-slate-200">
          <textarea
            value={draftOrder}
            onChange={(event) => setDraftOrder(event.target.value)}
            rows={2}
            placeholder="Type a test order..."
            className="min-h-[52px] flex-1 resize-none bg-transparent px-2 py-1.5 text-sm font-medium text-slate-900 outline-none placeholder:text-slate-400"
          />
          <button
            type="submit"
            disabled
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-slate-200 text-slate-400"
            aria-label="Order builder coming next"
          >
            <Send className="h-4 w-4" />
          </button>
        </div>

        <div className="mt-2 text-center text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">
          Order builder comes next
        </div>
      </form>

      {ticketOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/42 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-[28px] bg-white p-4 text-slate-950 shadow-[0_24px_70px_rgba(15,23,42,0.28)] ring-1 ring-white/80">
            ...
            <div className="mt-4 rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-200/80">
              ...
              <div className="mt-2 text-sm leading-5 text-slate-500">
                The next patch will turn typed orders into ticket tiles.
              </div>
            </div>
