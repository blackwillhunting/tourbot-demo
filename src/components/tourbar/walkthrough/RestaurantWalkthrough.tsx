...
type ExactMobileSocialIntroPhase =
  | "empty"
  | "searchbar"
  | "site"
  | "smartbar"
...
function ExactMobileSocialFilmBackground() {
  return (
    <>
...
function ExactMobileSocialFilmHeader({ phase }: { phase: ExactMobileSocialIntroPhase }) {
  return (
    <motion.div
      initial={false}
...
function ExactMobileSocialIntro({ runId }: { runId: number }) {
  const [phase, setPhase] = useState<ExactMobileSocialIntroPhase>("empty");
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setPhase("empty");
      await new Promise<void>((resolve) => window.setTimeout(resolve, 790));
      if (cancelled) return;

      setPhase("searchbar");
      await new Promise<void>((resolve) => window.setTimeout(resolve, 1010));
      if (cancelled) return;
      ...
      setPhase("title");
      await new Promise<void>((resolve) => window.setTimeout(resolve, 2420));
      if (cancelled) return;
      ...
      : phase === "searchbar"
        ...
        : phase === "site"
          ...
          : phase === "smartbar" || shellMounted
            ...
            : null;
  const demoSubmission: SmartBarMobileDemoSubmission | null =
    phase === "entry"
      ? {
          id: runId + 1,
  ...
  return (
    <div className="absolute inset-0 isolate overflow-hidden bg-[#eff8ff] text-white">
      <ExactMobileSocialFilmBackground />
      <ExactMobileSocialFilmHeader phase={phase} />
...
function CustomerFlowScene({
  ...
  if (isEmbeddedPhone && isCapsuleStep) {
    return (
      <div className="relative h-full overflow-hidden bg-[#eff8ff] text-slate-950">
        <ExactMobileSocialIntro runId={runId} />
